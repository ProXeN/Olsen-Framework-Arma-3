...
call compileFinal preprocessFileLineNumbers "modules\zbe_cache\zbe_functions.sqf";
	...
	while {true} do {
		...
		zbe_players = (switchableUnits + playableUnits);
		{
			...
			if (!_disable && !(_x in zbe_cachedGroups)) then {
				 ...
				 [zbe_aiCacheDist, _x, zbe_minFrameRate, zbe_debug] execFSM "modules\zbe_cache\zbe_aiCaching.fsm";
				};
		} forEach allGroups;
	};
};
		...
		_assetscar = zbe_centerPOS nearEntities ["LandVehicle", zbe_mapside];
		{
			if !(_x in zbe_cached_cars) then {
				zbe_cached_cars = zbe_cached_cars + [_x];
					[_x, zbe_vehicleCacheDistCar] execFSM "modules\zbe_cache\zbe_vehicleCaching.fsm";
			};
		} forEach _assetscar;
		_assetsair = zbe_centerPOS nearEntities ["Air", zbe_mapside];
		{
			if !(_x in zbe_cached_air) then {
				zbe_cached_air = zbe_cached_air + [_x];
				    [_x, zbe_vehicleCacheDistAir] execFSM "modules\zbe_cache\zbe_vehicleCaching.fsm";
			};
		} forEach _assetsair;
		_assetsboat = zbe_centerPOS nearEntities ["Ship", zbe_mapside];
		{
			if !(_x in zbe_cached_boat) then {
				zbe_cached_boat = zbe_cached_boat + [_x];
					[_x, zbe_vehicleCacheDistBoat] execFSM "modules\zbe_cache\zbe_vehicleCaching.fsm";
			};
		} forEach _assetsboat;
...
// if (!isDedicated) then {execFSM "modules\zbe_cache\zbe_clientObjectDrawAuto.fsm";};